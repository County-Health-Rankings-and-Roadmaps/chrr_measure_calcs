---
title: "NCHS v127 - Premature age-adjusted mortality - R2026"
author: "GL"
...
2023 Census Population Estimates: https://www2.census.gov/programs-surveys/popest/datasets/2020-2023/counties/asrh/cc-est2023-alldata.csv
...
```{r}
process_pop_v127 <- function(year, by_race_TF = FALSE){
...
```{r}
process_mort_v127 <- function(year, by_race_TF = FALSE){
...
## Census pop data 2021-2023 by age cat 1-8 
...
v127_1 <- merged_2 %>% 
  ...
  mutate(across(starts_with("v"), ~ if_else(!is.na(flag2) & flag2 == 2, 
                                            NA_real_, .x))) %>% 
  mutate(test_cb=round((v127_rawvalue**2)/(stdev**2), digits = 0))
...
v127_2 <- v127_1 %>% 
  ...
  mutate(v127_cilow = if_else(v127_numerator>=20 & v127_numerator <=99,
                         v127_rawvalue * L, v127_cilow),
         v127_cihigh = if_else(v127_numerator>=20 & v127_numerator <=99,
                         v127_rawvalue * U, v127_cihigh)) %>% 
  ...
  mutate(v127_sourceflag = NA_real_)
...
```{r}
v127_3 <- v127_2 %>% 
  add_flag_CT(vnum = "v127", old="A", new = "U")
```


# Compare to initial calculations 
```{r}
compare("v127", v127_3, year = 2026)
```
