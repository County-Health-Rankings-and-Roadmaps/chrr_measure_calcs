---
title: "NCHS v161 Suicide (subgroups) - R2027"
...
- NCHS mortality and natality data 2020-2024: the data is requested

- Census population estimates 2020-2024 from:
...
```{r}
source("../calculations/nchs_measure_helpers.R")
```

## function: process pop data for v161 subgroups

```{r}
process_pop_v161_other <- function(year, by_race_TF = TRUE){
...
## function: process mortality data for v161 subgroups

```{r}
process_mort_v161_other <- function(year, by_race_TF = TRUE, icd_v161 = NULL){
...
infant_pop <- 
  get_infant_pop(
    yrs, 
    file.path(nchs_nata_dir, "nchs_births_2018_2024.csv"), 
    by_race = TRUE) %>% 
  glimpse()
...
## Census pop data by race and age cat 1-10 for 2020-2024

```{r}
pop_agecat_1_10 <- bind_rows(
  process_pop_v161_other(2020), 
  process_pop_v161_other(2021),
  process_pop_v161_other(2022) %>% fix_CT_pop(process_pop_v161_other(2021)),
  process_pop_v161_other(2023) %>% fix_CT_pop(process_pop_v161_other(2021)),
  process_pop_v161_other(2024) %>% fix_CT_pop(process_pop_v161_other(2021))
  ) %>% 
...
## pop_all: add infant pop, fix age_cat 1

- Note: pop for age_cat 0 and 1 may not be needed, as no case of suicides was found in the mortality data. This step is included to be consistent with other calculations.
...
```{r}
mort_all <- bind_rows(
  process_mort_v161_other(2024, icd_v161 = icd_code_lst), 
  process_mort_v161_other(2023, icd_v161 = icd_code_lst), 
  process_mort_v161_other(2020, icd_v161 = icd_code_lst),
  process_mort_v161_other(2021, icd_v161 = icd_code_lst),
  process_mort_v161_other(2022, icd_v161 = icd_code_lst)
  ) %>%
...
-   no cases for age_cat 0 and 1 (age 0, age 1-4)
```{r}
mort_all %>% 
  distinct(age_cat)
```
...
v161_2 <- v161_1 %>% 
  ...
  mutate(cilow = if_else(numerator>=10 & numerator <=99,
         ...
         cihigh = if_else(numerator>=10 & numerator <=99,
                         rawvalue * U, cihigh)) %>% 
  mutate(flag2 = if_else(relative_standard_error > 0.25 & flag2!=2, 
                         1, flag2)) %>% 
...
```{r}
v161_other <- process_race_data_wide(v161_2, fips, "v161") %>% 
...
```{r}
v161_other_2 <- v161_other %>% 
  add_flag_CT(vnum = "v161_race", old="A", new = "U")
...
```{r}
compare_subgroup("v161", v161_other_2, 2027)
```
...
# save data
```{r}
save_data(v161_other_2, "v161_otherdata")
```
